/**
 * @fuse/skideancer-ai-agent - Code Analysis Capability
 * Ported from SkIDEancer's advanced code analysis
 *
 * Features:
...
export class CodeAnalysisCapability implements IAgentCapability {
  ...
  readonly description =
    'Analyzes code for patterns, issues, security vulnerabilities, and provides suggestions with auto-fixes';
  ...
  async generateFixes(suggestion: ICodeSuggestion, code: string): Promise<ICodeFix[]> {
  ...
  private formatAnalysisResult(result: ICodeAnalysisResult): string {
    ...
    if (result.suggestions.length > 0) {
      ...
      for (const sug of result.suggestions) {
        const icon = sug.severity === 'hint' ? '💡' : sug.severity === 'info' ? 'ℹ️' : '⚠️';
