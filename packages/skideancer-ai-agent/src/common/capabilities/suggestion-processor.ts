...
import { IAgentCapability, IAgentContext, IAgentResponse, ICodeEdit, IRange } from '../types';
...
export class SuggestionProcessorCapability implements IAgentCapability {
  ...
  private generateImportSuggestions(text: string, languageId: string): IProcessedSuggestion[] {
    const suggestions: IProcessedSuggestion[] = [];
  ...
  private needsImport(text: string, existingImports: string[]): boolean {
    ...
    return commonPatterns.some(
      (p) => text.includes(p) && !existingImports.some((i) => i.includes(p.replace('_.', 'lodash')))
    );
  }
  ...
  private needsDocumentation(code: string, lineNumber: number): boolean {
    ...
    const needsDocs = /^(?:export\s+)?(?:async\s+)?(?:function|class|interface|type)\s+\w+/.test(
      line.trim()
    );
